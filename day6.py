...
def ExtractAnswersForGroup(data):
    groups_anwsers = [{'answers':defaultdict(lambda: 0)}]
    group_count = 0
    #this is ugly but couldnot be botterd to proparly read the data 
    number_of_people = -1
    for line in data.split("\n"):
        number_of_people += 1        
        if line:
            for letter in line:        
                groups_anwsers[group_count]['answers'][letter] += 1
            continue

        groups_anwsers[group_count]['nbr_people'] = number_of_people            
        number_of_people = -1
        group_count += 1
        groups_anwsers.append({'answers':defaultdict(lambda: 0)})   
    else:
         groups_anwsers[group_count]['nbr_people'] = number_of_people    
    return groups_anwsers

    
def CountOccurancesTotalAnswers(groups_anwsers):
    count = sum([ len(group['answers'].keys()) for group in groups_anwsers])
    return count


def CountOccurancesSharedByEveryOneTotal(groups_anwsers):
    total_count = 0
    for group in groups_anwsers:
        nbr_of_people = group['nbr_people']
        total_count += sum([1 if count_answers == nbr_of_people else 0 for count_answers in group['answers'].values()])
    return total_count


def main():
    answers_per_group = ExtractAnswersForGroup(get_data())
    print(CountOccurancesTotalAnswers(answers_per_group))
    print(CountOccurancesSharedByEveryOneTotal(answers_per_group))
