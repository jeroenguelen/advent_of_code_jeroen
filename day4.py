...
data = [ """eyr:2021 hgt:168cm hcl:#fffffd pid:180778832 byr:1923 ecl:amb iyr:2019 cid:241
    ...
    for line in split_data:
        if line:
            for items in line.split(" "):
                ...
                passports[current_pos].update({key:value})
        else:
            passports.append({})
            current_pos +=1
...
def check_passports(passports):
    hair_color_pattern = re.compile("(\#(([a-f]|[0-9]){6}))")
    ...
    should_contain_keys = {
        ...
        "hcl":lambda x: hair_color_pattern.search(x) and len(x) == 7,
        "ecl": lambda x: x in eye_colors,
        "pid": lambda x: pid_pattern.search(x) and len(x) == 9,
    }        
        
    not_valid_passports = 0
    for passport in passports:
        for key, validation_fnc in should_contain_keys.items():
            if key not in passport or not validation_fnc(passport[key]):
                not_valid_passports += 1
                break
    print("valid passports = {}".format(len(passports) - not_valid_passports))



def test():
    _input = [("150cm", True), ("151cm", True), ("149cm", False), ("190cm",True),("194cm",False)]

    for i, g in _input:
        assert hight_check(i) == g
    ...
    _input = [("#abf093", True), ("#abz093", False)]
    for i, g in _input:        
        assert should_contain_keys["hcl"](i) == g

    _input = [("123456789", True), ("0123456789", False)]
    for i, g in _input:       
        assert should_contain_keys["pid"](i) == g

    _input = [("a", False), ("amb", True)]
    for i, g in _input:       
        assert should_contain_keys["ecl"](i) == g

    _input = [("0123456789", False), ("amb", False)]
    for i, g in _input:
...
test()
check_passports(extract_data(data[0]))
